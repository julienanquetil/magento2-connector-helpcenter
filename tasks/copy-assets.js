...
gulp.task('copy-assets', ['clean-dist'], function(){
    var lib = gulp.src([
        ...
        .pipe(gulp.dest('dist/magento2-connector/' + majorVersion + '/js/'));
    var css = gulp.src(['node_modules/docsearch.js/dist/cdn/docsearch.min.css'])
        .pipe(gulp.dest('dist/magento2-connector/' + majorVersion + '/css/'));
    var fonts = gulp.src(['node_modules/typeface-comfortaa/files/*',
        ...
        .pipe(gulp.dest('dist/magento2-connector/' + majorVersion + '/fonts/'));
    var img = gulp.src('content/img/**/*')
        .pipe(rename({dirname: ''}))
        .pipe(gulp.dest('dist/magento2-connector/' + majorVersion + '/img/'));
    var img = gulp.src('content/pdf-user-guides/**/*')
        .pipe(rename({dirname: ''}))
        .pipe(gulp.dest('dist/magento2-connector/' + majorVersion + '/pdf-user-guides/'));
